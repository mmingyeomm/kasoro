...
describe("turtle_anchor", () => {
  ...
  // DAO PDA 생성
  const [daoPDA, _] = PublicKey.findProgramAddressSync(
    [Buffer.from("dao"), initializer.toBuffer()],
    program.programId
  );
  ...
  it("DAO에 SOL을 예치합니다", async () => {
    try {
      ...
      const amount2 = new anchor.BN(0.2 * LAMPORTS_PER_SOL); // 0.1 SOL
      console.log("예치할 금액:", amount.toString(), "lamports");
